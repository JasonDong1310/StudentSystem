...
class AddteaFrame extends JFrame implements ActionListener
{
	...
	public AddteaFrame()
	{
		...
		JLabel lbdept = new JLabel("               学  院:");
		jtdept = new JTextField(10);
		JLabel lbmajor = new JLabel("              专  业:");
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getActionCommand().equals("btn"))
		{
			...
			if (resOne == 0)
			{
				...
				try
				{
					String sql = "INSERT INTO NewStudent(Sno,Sname,Ssex,Sdept,Smajor,Password)values('" + sno + "','"
							+ n + "','" + s + "','" + d + "','" + m + "','" + p + "')";
					Jdbc.insert(sql);
					this.dispose();
				} catch (Exception x)
...
class AddteaFrame2 extends JFrame implements ActionListener
{
	...
	String sql;
	StudentinfoModel sm;
	String sno, name, sex, dept, major, pwd,bj;// Grade1,Grade2,Grade3,TotalGrade,class;
	/* 修改学生信息界面 */
...
//		this.bj = bj;
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getActionCommand().equals("btn"))
		{
			int resOne = JOptionPane.showConfirmDialog(new AddteaFrame2(sno, name, sex, dept, major, pwd), "确定修改", "账号",
			...
			if (resOne == 0)
			{
				...
				try
				{
					String sql = "update NewStudent set Sname='" + n + "',Ssex='" + x + "',Sdept='" + d + "',Smajor='"
							+ m + "',Password='" + p + "' where Sno='" + sno + "'";
					Jdbc.insert(sql);
					this.dispose();
