...
class AddstuFrame extends JFrame implements ActionListener
{
	...
	JTextField jtname, jtsex, jtdept, jtmajor, jtsno, jtpwd, jtpwd1,jtclass;
	String sql;
	StudentinfoModel sm;
	...
	public AddstuFrame()
	{
		...
		JLabel lbsex = new JLabel("          语文成绩:");
		jtsex = new JTextField(10);
		JLabel lbdept = new JLabel("          数学成绩:");
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			int resOne = JOptionPane.showConfirmDialog(this, "确定增加成绩信息", "成绩", JOptionPane.OK_CANCEL_OPTION);
			if (resOne == 0)
			{
				...
				String b = jtclass.getText().trim();
				try
				{
					String sql = "INSERT INTO gradestudent(Sno,Sname,Grade1,Grade2,Grade3,TotalGrade,class)values('" + sno
							+ "','" + n + "','" + s + "','" + d + "','" + m + "','" + p + "','" + b + "')";
					Jdbc.insert(sql);
					this.dispose();
				} catch (Exception x)
...
class AddstuFrame2 extends JFrame implements ActionListener
{
	...
	String sql;
	StudentinfoModel sm;
	String sno, name, sex, dept, major, pwd,bj;// Grade1,Grade2,Grade3,TotalGrade;

	/* 修改成绩信息界面 */
	public AddstuFrame2(String sno, String name, String sex, String dept, String major, String pwd,String bj)
	{
		this.sno = sno;
		this.name = name;
		this.sex = sex;
		...
		JLabel lbclass = new JLabel("             班级");
		jtclass = new JTextField(bj,10);
		...
		jtpwd.setFont(new Font("Dialog", 0, 22));
		jtsno.setFont(new Font("Dialog", 0, 22));
		jtclass.setFont(new Font("Dialog",0,22));
		// 设置标签字体大小
		lbsno.setFont(new Font("Dialog", 1, 22));
		lbname.setFont(new Font("Dialog", 1, 22));
		lbsex.setFont(new Font("Dialog", 1, 22));
		...
		lbpwd.setFont(new Font("Dialog", 1, 22));
		lbmajor.setFont(new Font("Dialog", 1, 22));
		lbclass.setFont(new Font("Dialog",1,22));
		// 添加组件到面板
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getActionCommand().equals("btn1"))
		{
			int resOne = JOptionPane.showConfirmDialog(new AddstuFrame2(sno, name, sex, dept, major, pwd,bj), "确定修改", "成绩",
			...
			if (resOne == 0)
			{
				...
				try
				{
					String sql = "update gradestudent set Sno='" + s + "',Sname='" + n + "',Grade1='" + x + "',Grade2='"
							+ d + "',Grade3='" + m + "',class='" + b + "',TotalGrade='" + p + "' where Sno='" + sno + "'";
					Jdbc.insert(sql);
					this.dispose();
				} catch (Exception ss)
				{
