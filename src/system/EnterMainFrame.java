...
//		 JLabel j4 = new JLabel(image);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getActionCommand().equals("enterbtn") && studentbtn.isSelected())
		{
			...
			try
			{
				sql = "select Sno,Password,Sname from NewStudent";
...
//				 frame.setVisible(true);
				 JOptionPane.showMessageDialog(null,"登入成功","", JOptionPane.INFORMATION_MESSAGE);
			} else
			{
				uTxt.setText("");
				pTxt.setText("");
		...
		if (e.getActionCommand().equals("enterbtn") && teacherbtn.isSelected())
		{
			try
			{
				sql = "select ID,Password,Sname from NewTeacher";
				Connection con = Jdbc.getconnection();
				Statement stmt = con.createStatement();
				ResultSet rs = stmt.executeQuery(sql);
				while (rs.next())
...
class ZcFrame extends JFrame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			int resOne = JOptionPane.showConfirmDialog(new ZcFrame(), "确定增加", "账号", JOptionPane.OK_CANCEL_OPTION);
			if (resOne == 0)
			{
				try
				{
					String sql = "INSERT INTO NewStudent(Sno,Password)values('" + sno + "','" + p + "')";
					Jdbc.insert(sql);
					this.dispose();
					EnterMainFrame enter = new EnterMainFrame();
					enter.setVisible(true);
		...
		} else if (p.equals(p1) && e.getActionCommand().equals("btn2"))
		{
			...
			if (resOne == 0)
			{
				try
				{
					String sql = "INSERT INTO NewTeacher(ID,Password)values('" + sno + "','" + p + "')";
					Jdbc.insert(sql);
					this.dispose();
					EnterMainFrame enter = new EnterMainFrame();
					enter.setVisible(true);
